$(function(){
    const dtTable = window.LaravelDataTables["pengajuan-table"]

    $('.btn-add-pengajuan').on('click', function(){
        $('.modal-title').text('Add pengajuan')
    })
    ...
    $('table.dataTable tbody').on('click', 'tr td .btn-edit', function(){
        ...
        $.ajax({
            type: 'POST',
            url: `${baseUrl}/pengajuan/${t.data('id')}/edit`,
            data: {id:t.data('id')},
            beforeSend: function() {
    ...
    $('form').on('submit', function(e){
        ...
        const actionUrl = id ? `${baseUrl}/pengajuan/${id}/edit` : `${baseUrl}/pengajuan`
