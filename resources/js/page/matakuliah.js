$(function(){
    const dtTable = window.LaravelDataTables["matakuliah-table"]

    $('.btn-add-matakuliah').on('click', function(){
        $('.modal-title').text('Add matakuliah')
    })
    ...
    $('table.dataTable tbody').on('click', 'tr td .btn-edit', function(){
        ...
        $.ajax({
            type: 'POST',
            url: `${baseUrl}/matakuliah/${t.data('id')}/edit`,
            data: {id:t.data('id')},
            beforeSend: function() {
    ...
    $('form').on('submit', function(e){
        ...
        const actionUrl = id ? `${baseUrl}/matakuliah/${id}/edit` : `${baseUrl}/matakuliah`
        ...
        $.ajax({
            ...
            processData: false,
            beforeSend: function() {
                saveBtn.html('<div class="spinner-border spinner-border-sm" matakuliah="status"></div>').prop('disabled', true)
